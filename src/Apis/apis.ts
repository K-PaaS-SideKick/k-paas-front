...
import { JoinResponse, StatusResponse, SearchResponse, NewProjectResponse, MainPageResponse } from '../Interfaces/project';
...
// 프로젝트 검색 api
export const searchProjects = async (titleQuery: string, pageable: { page: number; size: number; sort: string[] }): Promise<SearchResponse> => {
    try {
        const response: AxiosResponse<SearchResponse> = await api.post('/search', null, {
            params: {
                titleQuery,
                pageable,
            },
...
// 새 프로젝트 작성 api
export const createNewProject = async (projectDTO: object, images: File[]): Promise<NewProjectResponse> => {
    try {
        ...
        const response: AxiosResponse<NewProjectResponse> = await api.post('/new', formData, {
            headers: {
...
// 메인 페이지 호출
export const getMainPageProjects = async (pageable: { page: number; size: number; sort: string[] }): Promise<MainPageResponse> => {
    try {
  ...
  export const updateProject = async (projectData: { pid: number; uid: string; title: string; content: string; status: string; repo_link: string; category: number[]; current_members: number; max_members: number; images: string[] }): Promise<NewProjectResponse> => {
    try {
      const response: AxiosResponse<NewProjectResponse> = await api.patch('/', projectData);
      return response.data;
    } catch (error) {
